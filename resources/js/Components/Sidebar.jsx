        ...
        <>
            ...
            >
                ...
                <div className="relative z-10 p-6 flex items-center justify-between border-b border-white/10">
                    <img src="/white_logo.webp" alt="Logo" className="h-10 w-auto" />
                    <button
                        onClick={onClose}
                ...
                <nav className="relative z-10 flex-1 overflow-y-auto py-4 px-3 space-y-1">
                    <SidebarLink href="/main" active={url === "/main"}>
                        <i className="bi bi-house-door text-lg"></i>
                        <span>Home</span>
                    </SidebarLink>

                    <SidebarLink href={route('student.info.filter')} active={route().current('student.info.filter')}>
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                        <span>Student Information</span>
                    </SidebarLink>

                    <SidebarLink href={route('academic.profile.filter')} active={route().current('academic.profile.filter')}>
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg>
                        <span>Academic Profile</span>
                    </SidebarLink>

                    <SidebarLink href={route('program.metrics.filter')} active={route().current('program.metrics.filter')}>
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 002 2h2a2 2 0 002-2z"></path></svg>
                        <span>Program Metrics</span>
                    </SidebarLink>

                    <SidebarLink href={route('report.filter')} active={route().current('report.filter')}>
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                        <span>Generate Report</span>
                    </SidebarLink>

                    <SidebarLink href="/student-additional" active={url.startsWith("/student-additional")}>
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        <span>Additional Entry</span>
                    </SidebarLink>

                    {/* RBAC: Only show to authorized personnel */}
                    {canManageSystem && (
                        <>
                            ...
                            <SidebarLink href="/transaction-logs" active={url.startsWith("/transaction-logs")}>
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                <span>Transaction Logs</span>
                            </SidebarLink>

                            <SidebarLink href="/users" active={url.startsWith("/users")}>
                ...
                <div className="relative z-30 border-t border-white/10 p-4 bg-[#5c297c]">
                    ...
                    >
                        ...
                        <div className="w-9 h-9 rounded-full bg-white/20 flex items-center justify-center text-sm font-bold shrink-0 border border-white/30 overflow-hidden">
                            {user.avatar ? (
                                <img 
                                    src={user.avatar} 
                        ...
                        <div className="text-left flex-1 min-w-0">
                            ...
                            <div className="text-xs text-white/60 truncate uppercase">
                                {user.position}
                            </div>
                        </div>
                        <svg className={`w-4 h-4 text-white/50 transition-transform duration-300 ${isProfileOpen ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 15l7-7 7 7"></path></svg>
                    </button>
                </div>
            </aside>
        </>
