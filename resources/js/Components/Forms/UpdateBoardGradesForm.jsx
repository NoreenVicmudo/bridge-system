...
export default function UpdateBoardGradesForm({
    data,
    setData,
    errors = {},
    processing = false,
    ...
    // Form is valid if a subject is selected and a grade is entered
    const isFormValid = data.subject_id && data.subject_grade !== "";

    // Auto-fill existing grade when a subject is selected
    useEffect(() => {
        if (data.subject_id) {
            const existingGrade = currentGrades[data.subject_id] || "";
            setData("subject_grade", existingGrade);
        }
    }, [data.subject_id]);
        ...
        <div className="w-full font-montserrat text-left text-[#5c297c]">
            <form onSubmit={(e) => { e.preventDefault(); if (isFormValid) setIsModalOpen(true); }} className="flex flex-col gap-4">
                ...
                <div className="border-t border-gray-100 pt-4">
                    <label className={labelClass}>Board Subjects Overview:</label>
                        ...
                        {subjectOptions.length > 0 ? (
                            subjectOptions.map((sub) => (
                                <div key={sub.value} className="flex gap-2 animate-fade-in">
                                    ...
                                    <input
                                        type="text"
                                        value={currentGrades[sub.value] ? `${currentGrades[sub.value]}%` : "N/A"}
                                        className={`${inputClass} font-bold text-center w-24 bg-gray-50 border-gray-200 ${!currentGrades[sub.value] ? "text-gray-300" : "text-[#5c297c]"}`}
                                        readOnly
                                    />
                                </div>
                            ))
                            ...
                            <p className="text-gray-400 italic text-sm py-2">
                                No board subjects mapped to this program.
                            </p>
                        )}
                    </div>
                </div>
                ...
                <div className="border-t border-gray-100 pt-5 bg-purple-50/30 p-4 rounded-lg mt-2">
                    <h3 className="text-[11px] font-bold mb-4 uppercase tracking-widest opacity-70">Update Subject Grade</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <TextInput
                                ...
                                onChange={(e) => setData("subject_grade", e.target.value)}
                                className={inputClass}
                                required
                            />
                            {errors.subject_grade && <div className="text-red-500 text-xs mt-1">{errors.subject_grade}</div>}
                        </div>
                    </div>
                </div>
            ...
            <ConfirmSaveModal
                ...
                message={
                    <>
                        Are you sure you want to update the board subject grade for <br />
