...
export default function CreateUserForm({
    ...
    // 🧠 FIXED: Extracted handlers to prevent React Synthetic Event Nullification
    ...
    const handleCollegeChange = (e) => {
        ...
        setData(prev => ({
            ...
            program_id: "" // Instantly reset program when college changes!
        }));
    };

    // 🧠 FIXED: Bulletproof filtering mimicking StudentInfoFilter
    ...
    const isFormValid =
        ...
        data.password.trim() !== "" && data.position !== "" &&
    ...
    return (
        <div className="flex justify-center w-full px-4 py-0 h-[calc(100vh-100px)] items-start overflow-hidden font-montserrat text-left text-[#5c297c]">
            <div className="w-full max-w-2xl bg-white rounded-[10px] shadow-[0_6px_25px_rgba(0,0,0,0.1)] px-6 py-4 md:px-8 md:py-5 flex flex-col max-h-full my-2 relative">
                <h2 className="text-center text-xl md:text-2xl font-bold mb-3 flex-shrink-0">Create New User</h2>
                <div className="overflow-y-auto overflow-x-hidden pr-2 flex-1 custom-form-scrollbar">
                    ...
                    <form onSubmit={openConfirmModal} className="flex flex-col gap-3 p-1">
                        ...
                        <div className="bg-purple-50/30 p-4 rounded-lg border border-purple-100/50 space-y-4">
                            <h3 className="text-[11px] font-bold mb-2 uppercase tracking-widest opacity-70">Personal Information</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div className="w-full">
                                    <label className={labelClass}>First Name: <span className="text-red-500">*</span></label>
                                    <TextInput type="text" value={data.fname} onChange={(e) => setData("fname", e.target.value)} className={inputClass} placeholder="e.g. Juan" required />
                                    {errors.fname && <p className={errorClass}>{errors.fname}</p>}
                                </div>
                                <div className="w-full">
                                    <label className={labelClass}>Last Name: <span className="text-red-500">*</span></label>
                                    <TextInput type="text" value={data.lname} onChange={(e) => setData("lname", e.target.value)} className={inputClass} placeholder="e.g. Dela Cruz" required />
                        ...
                        <div className="bg-purple-50/30 p-4 rounded-lg border border-purple-100/50 space-y-4">
                            <h3 className="text-[11px] font-bold mb-2 uppercase tracking-widest opacity-70">Account Credentials</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div className="w-full">
                                    <label className={labelClass}>Username: <span className="text-red-500">*</span></label>
                                    <TextInput type="text" value={data.username} onChange={(e) => setData("username", e.target.value)} className={inputClass} placeholder="Enter unique username" required />
                                    {errors.username && <p className={errorClass}>{errors.username}</p>}
                                </div>
                                <div className="w-full">
                                    <label className={labelClass}>Email Address: <span className="text-red-500">*</span></label>
                                    <TextInput type="email" value={data.email} onChange={(e) => setData("email", e.target.value)} className={inputClass} placeholder="name@mcu.edu.ph" required />
                                    {errors.email && <p className={errorClass}>{errors.email}</p>}
                                </div>
                            </div>
                            <div className="w-full">
                                <label className={labelClass}>Password: <span className="text-red-500">*</span></label>
                                <TextInput type="password" value={data.password} onChange={(e) => setData("password", e.target.value)} className={inputClass} placeholder="Minimum 8 characters" required />
                                {errors.password && <p className={errorClass}>{errors.password}</p>}
                            </div>
                        </div>

                        <div className="bg-purple-50/30 p-4 rounded-lg border border-purple-100/50 space-y-4">
                            <h3 className="text-[11px] font-bold mb-2 uppercase tracking-widest opacity-70">Access & Roles</h3>
                            <div className="space-y-3">
                                <CustomSelectGroup
                                    ...
                                    placeholder="Select Position" vertical={true} className={selectGroupOverride} labelClassName={selectLabelOverride}
                                />
                                ...
                                {needsProgram && (
                                    <div className="animate-fade-in-up">
                                        ...
                                        />
                                        {errors.program_id && <p className={errorClass}>{errors.program_id}</p>}
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end gap-3 mt-4 mb-2 flex-shrink-0">
                            <button type="button" onClick={() => window.history.back()} className="px-6 py-2.5 text-sm font-bold text-[#666] bg-white border border-[#ddd] rounded-[6px] hover:bg-[#ffb736] hover:text-white transition-all duration-300 shadow-sm font-montserrat">Cancel</button>
                            <button type="submit" disabled={processing || !isFormValid} className={`px-6 py-2.5 text-sm font-bold text-white rounded-[6px] transition-all duration-300 shadow-md flex items-center gap-2 ${!isFormValid ? "bg-gray-400 cursor-not-allowed opacity-70" : "bg-[#5c297c] hover:bg-[#ffb736] cursor-pointer"}`}>
                                {processing ? <><div className="loader w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div> Saving...</> : "Create User"}
                            </button>
                        </div>
                    </form>
                </div>
                <ConfirmSaveModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onConfirm={submit} message={<>Are you sure you want to create a new account for <br /><strong>{data.username}</strong>?</>} />
            </div>
        </div>
    );
}