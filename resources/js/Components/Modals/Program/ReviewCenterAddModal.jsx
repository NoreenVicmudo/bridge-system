...
export default function ReviewCenterAddModal({ isOpen, onClose, currentFilter, onImportSuccess }) {
    ...
    const handleProceedToEdit = () => {
        // Sends the student number and all current batch filters to the edit route
        router.get(route('review.center.edit'), { 
    ...
    const handleImportSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(route('review-center.import'), formData, { 
    ...
    return (
        <div className={`fixed inset-0 z-[1000] flex items-center justify-center transition-all duration-300 ${animate ? "bg-gray-900/60 backdrop-blur-sm" : "bg-transparent backdrop-blur-none pointer-events-none"}`}>
            <div className={`bg-white rounded-2xl w-[90%] max-w-[500px] shadow-2xl relative overflow-hidden transition-all duration-300 transform ${animate ? "scale-100 opacity-100" : "scale-95 opacity-0"}`}>
                <div className="bg-[#5c297c] p-6 text-center text-white relative">
                    <h2 className="text-2xl font-bold tracking-wide">Manage Review Centers</h2>
                    <p className="text-purple-200 text-sm mt-1">Import CSV or update student records</p>
                    ...
                    {view === "import" && (
                        <div className="flex flex-col gap-5 animate-fade-in-up">
                            <div className="bg-purple-50 p-5 rounded-lg border border-purple-100 text-center">
                                ...
                                <p className="text-xs text-gray-600 mb-4">Format: [Student Number], [Full Name], [Review Center]</p>
                                <form onSubmit={handleImportSubmit} className="flex flex-col gap-3">
                                    <input type="file" accept=".csv" onChange={(e) => setImportFile(e.target.files[0])} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-purple-100 file:text-[#5c297c] hover:file:bg-purple-200 cursor-pointer" required />
                                    {importError && <p className="text-red-500 text-xs mt-1 font-medium">{importError}</p>}
                                    <button type="submit" disabled={importProcessing || !importFile} className="mt-2 w-full py-2.5 bg-[#5c297c] text-white font-bold rounded-lg hover:bg-[#4a1f63] transition-all shadow-md">
                                        {importProcessing ? "Processing..." : "Import CSV"}
                                    </button>
                                </form>
                            </div>
                        ...
                        <div className="flex flex-col gap-5 animate-fade-in-up">
                            <div className="bg-purple-50 p-4 rounded-lg border border-purple-100">
                                ...
                                <div className="flex gap-2">
                                    <input type="text" value={studentNumber} onChange={(e) => setStudentNumber(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleCheckStudent()} placeholder="e.g. 2025-1005" className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#5c297c] focus:border-transparent outline-none transition-all" />
                                    <button onClick={handleCheckStudent} disabled={!studentNumber.trim() || checkStatus === "loading"} className="px-6 py-2.5 bg-[#5c297c] text-white font-bold rounded-lg hover:bg-[#4a1f63] shadow-md transition-all">
                            ...
                            {checkStatus === "exists" && (
                                <div className="flex flex-col gap-3 items-center animate-fade-in">
                                    <div className="flex items-center gap-2 p-3 bg-blue-50 text-blue-700 w-full justify-center rounded-lg border border-blue-100">
                                        ...
                                        <span className="text-sm font-medium">Student found! Proceed to update center.</span>
                                    </div>
                                    <button onClick={handleProceedToEdit} className="w-full py-3 bg-[#ffb736] text-white font-bold rounded-lg shadow-md hover:bg-[#e0a800] hover:scale-[1.02] transition-all">Proceed to Entry</button>
                                </div>
                            )}

                            {checkStatus === "not_exists" && <div className="flex items-center gap-2 p-3 bg-red-50 text-red-700 w-full justify-center rounded-lg border border-red-100 animate-fade-in"><i className="bi bi-exclamation-triangle-fill text-xl"></i><span className="text-sm font-medium">Student not found in masterlist.</span></div>}
