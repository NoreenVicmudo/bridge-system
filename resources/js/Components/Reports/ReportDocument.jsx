...
const ReportDocument = forwardRef(
    ({ reportData, auth, collegeName, collegeLogo, collegeColor, collegeEmail }, ref) => {
        ...
        const PAGE_1_MAX_ROWS = 10; 
        const SUBSEQUENT_PAGE_MAX_ROWS = 19; 
        ...
        const renderFilters = () => {
            ...
            // Map common technical keys to readable labels
            const keyMap = {
                'college': 'College',
                'program': 'Program',
            ...
            // Explicitly extract the values you want
            Object.entries(reportData.filters).forEach(([key, val]) => {
                // Ignore internal technical keys or empty/ALL values
                if (key === 'mode' || key === '_token' || !val || val === 'ALL' || val === 'All') return;

                const label = keyMap[key] || key.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
                
                filterStrings.push(`${label}: ${val}`);
            });
            ...
            <div id="report" ref={ref} className="flex flex-col w-[816px] bg-transparent">
                ...
                <div className="report-page w-[816px] h-[1056px] bg-white shadow-lg flex flex-col box-border mb-8 print:mb-0 print:shadow-none break-after-page shrink-0 mx-auto relative overflow-hidden">
                    ...
                    <section className="flex-1 px-12 py-2 flex flex-col overflow-hidden">
                        ...
                        <div className="font-serif text-[15px] text-gray-600 mb-6 leading-relaxed shrink-0 flex justify-between">
                            <div>
                        ...
                        <div className="font-serif text-[15px] text-gray-800 mb-6 leading-relaxed shrink-0">
                            <p>Statistical Tool: <span className="font-bold">{reportData.tool}</span></p>
                            <p className="whitespace-pre-line">Variables: {reportData.fields}</p>
                            
                            {/* Display active data filters */}
                            <p className="mt-2 text-gray-600 text-sm">
                                <span className="font-bold text-gray-800">Filters Applied: </span> 
                                {renderFilters()}
                            </p>
                        </div>
