...
const SortableHeader = ({ label, sortKey, currentSort, currentDirection, onSort, className = "" }) => {
...
export default function StudentMasterlist({ students }) {
    ...
    const handleSort = isBackendReady ? (col) => console.log("Sort backend by", col) : mock.handleSort;
            ...
            <div className="relative" ref={dropdownRef}>
                ...
                <div className={`absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-[0_10px_40px_-10px_rgba(0,0,0,0.2)] border border-gray-100 overflow-hidden z-[999] transition-all duration-300 origin-top-right transform ${isOpen ? 'opacity-100 scale-100 translate-y-0 visible' : 'opacity-0 scale-95 -translate-y-2 invisible'}`}>
                    <div className="p-1 flex flex-col">
                        <a href={`/students/export/csv${queryParams}`} className="flex items-center gap-3 px-4 py-3 text-sm text-gray-600 hover:bg-purple-50 hover:text-[#5c297c] transition-colors rounded-md group">
                        ...
                        <a href={`/students/export/excel${queryParams}`} className="flex items-center gap-3 px-4 py-3 text-sm text-gray-600 hover:bg-purple-50 hover:text-[#5c297c] transition-colors rounded-md group">
                            <i className="bi bi-file-earmark-excel text-lg text-gray-400 group-hover:text-[#5c297c]"></i>
                            <span className="font-medium">Export to Excel</span>
                        </a>
                        <a href={`/students/export/pdf${queryParams}`} className="flex items-center gap-3 px-4 py-3 text-sm text-gray-600 hover:bg-purple-50 hover:text-[#5c297c] transition-colors rounded-md group">
    ...
    const renderFooterButtons = () => {
        if (!isRemoveMode) {
            return (
                <>
                    <button onClick={() => setIsAddModalOpen(true)} className="px-6 py-2 bg-[#5c297c] text-white rounded-[5px] text-sm font-medium hover:bg-[#4a1f63] transition-all duration-300 ease-in-out shadow-sm">Add Student</button>
                    <button onClick={() => setIsRemoveMode(true)} className="px-6 py-2 bg-[#5c297c] text-white rounded-[5px] text-sm font-medium hover:bg-[#ed1c24] transition-all duration-300 ease-in-out shadow-sm">Remove Student</button>
                </>
            );
        } else {
            return (
                <>
                    <button onClick={cancelRemoveMode} className="px-6 py-2 bg-white text-gray-600 border border-gray-300 rounded-[5px] text-sm font-medium hover:bg-gray-100 transition-all duration-300 ease-in-out shadow-sm">Cancel</button>
                    {selectedIds.size > 0 && <button onClick={() => setIsRemoveModalOpen(true)} className="px-6 py-2 bg-[#ed1c24] text-white rounded-[5px] text-sm font-medium hover:bg-[#c4151c] transition-all duration-300 ease-in-out shadow-sm animate-fade-in">Remove ({selectedIds.size})</button>}
                </>
            );
        }
    };
    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="py-8 px-4 sm:px-6 lg:px-8 bg-gray-50 min-h-screen">
                ...
                >
                        ...
                        <tr className="bg-[#5c297c] text-white text-sm uppercase leading-normal">
                            {isRemoveMode && (
                                <th className="py-3 px-6 font-bold text-center w-[50px] animate-fade-in">
                                    <input type="checkbox" onChange={toggleSelectAll} className="accent-[#5c297c] cursor-pointer w-4 h-4 transition-all duration-300 ease-in-out" />
                                </th>
                            )}
                            ...
                            <th className="py-3 px-6 font-bold">Permanent Address</th>
                            <th className="py-3 px-6 font-bold">Living Arrangement</th>
                    ...
                    <tbody className="text-gray-600 text-sm font-medium">
                        {data.data.length > 0 ? (
                                ...
                                <tr key={student.id} className={`border-b border-gray-100 hover:bg-purple-50 transition-all duration-300 ease-in-out ${index % 2 === 0 ? "bg-white" : "bg-[#efeded]"}`}>
                                    {isRemoveMode && (
                                        <td className="py-3 px-6 text-center animate-fade-in">
                                            <input type="checkbox" checked={selectedIds.has(student.id)} onChange={() => toggleSelection(student.id)} className="accent-[#5c297c] cursor-pointer w-4 h-4 transition-all duration-300 ease-in-out" />
                                        </td>
                                    )}
                                    <td className="py-3 px-6">
                                        <Link href={`#edit/${student.id}`} className="inline-block px-4 py-1.5 rounded-[6px] bg-[#ffb736] text-white font-bold hover:bg-[#e0a800] hover:scale-105 hover:shadow-md transition-all duration-300 ease-in-out text-center min-w-[100px]">
                                    ...
                                    <td className="py-3 px-6 font-bold">
                                        <span className={student.socioeconomic === "POOR" ? "text-[#ed1c24]" : "text-gray-600"}>{student.socioeconomic}</span>
                                    </td>
                                    <td className="py-3 px-6 uppercase min-w-[300px]">{student.address}</td>
                            ...
                            <tr>
                                <td colSpan={isRemoveMode ? 15 : 14} className="py-8 text-center text-gray-500 italic">No students found matching your search.</td>
                            </tr>
                        )}
                    </tbody>
                </TableContainer>
                ...
                <RemoveStudentModal isOpen={isRemoveModalOpen} onClose={() => setIsRemoveModalOpen(false)} selectedStudents={getSelectedStudents()} />
            </div>
        </AuthenticatedLayout>
    );
}