...
import { useMockInertia, MOCK_STUDENTS_RETAKES, MOCK_RETAKE_SUBJECTS } from "@/Hooks/useMockInertia";
...
export default function RetakesInfoPage({ students }) {
    const isBackendReady = !!students;
    const mock = useMockInertia(MOCK_STUDENTS_RETAKES);
    ...
    const subjectHeaders = isBackendReady ? students.subjects : MOCK_RETAKE_SUBJECTS;
    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="py-8 px-4 sm:px-6 lg:px-8 bg-gray-50 min-h-screen">
                                ...
                                {subjectHeaders.map((subject, idx) => {
                                    ...
                                    return (
                                        <td key={idx} className="py-3 px-6 text-center">
                                            <span className={`font-bold ${value > 0 ? "text-red-500" : "text-gray-600"}`}>{value}</span>
                                        </td>
                                    );
                                })}
                            </tr>
                ...
                <ChangeMetricModal isOpen={isMetricModalOpen} onClose={() => setIsMetricModalOpen(false)} currentMetric="Back Subjects/Retakes" />
