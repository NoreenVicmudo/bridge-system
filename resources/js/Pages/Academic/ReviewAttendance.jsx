...
import { useMockInertia, MOCK_STUDENTS_ATTENDANCE } from "@/Hooks/useMockInertia";
...
export default function ReviewAttendancePage({ students }) {
    const isBackendReady = !!students;
    const mock = useMockInertia(MOCK_STUDENTS_ATTENDANCE);
    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="py-8 px-4 sm:px-6 lg:px-8 bg-gray-50 min-h-screen">
                ...
                >
                    <thead>
                        <tr className="bg-[#5c297c] text-white text-sm uppercase leading-normal">
                            ...
                            <SortableHeader label="Sessions Attended" sortKey="attended" currentSort={sortColumn} currentDirection={sortDirection} onSort={handleSort} className="text-center" />
                            <SortableHeader label="Total Sessions" sortKey="total" currentSort={sortColumn} currentDirection={sortDirection} onSort={handleSort} className="text-center" />
                        </tr>
                    </thead>
                    <tbody className="text-gray-600 text-sm font-medium">
                        {data.data.length > 0 ? data.data.map((student, i) => (
                            <tr key={student.id} className={`border-b border-gray-100 hover:bg-purple-50 transition-all duration-300 ease-in-out ${i % 2 === 0 ? "bg-white" : "bg-[#efeded]"}`}>
                                ...
                                <td className="py-3 px-6 text-center font-bold text-[#5c297c]">{student.attended}</td>
                                <td className="py-3 px-6 text-center font-bold text-gray-600">{student.total}</td>
                            </tr>
                        )) : (
                            <tr><td colSpan={4} className="py-8 text-center text-gray-500 italic">No records found.</td></tr>
                        )}
                    </tbody>
                </TableContainer>
                ...
                <ChangeMetricModal isOpen={isMetricModalOpen} onClose={() => setIsMetricModalOpen(false)} currentMetric="Attendance in Review Classes" />
