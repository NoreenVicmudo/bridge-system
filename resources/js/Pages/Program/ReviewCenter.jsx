...
import ReviewCenterAddModal from "@/Components/Modals/Program/ReviewCenterAddModal"; // Use the new one
...
export default function ReviewCenterPage({ students, filter, search = "", sort = "", direction = "asc", dbColleges = [], dbPrograms = [] }) {
    ...
    // Enrich the filter object for the Info Card
    ...
    const handleSearch = (val) => {
        // FIXED: Changed 'review-center.index' to 'review.center'
        router.get(route('review.center'), { ...filter, search: val, sort, direction }, { preserveState: true });
    };
    ...
    const handleApplyFilter = (newFilters) => {
        // FIXED: Changed 'review-center.index' to 'review.center'
        router.get(route('review.center'), { ...newFilters, search, sort, direction }, { preserveState: true });
    };
    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="py-8 px-4 sm:px-6 lg:px-8 bg-gray-50 min-h-screen">
                        ...
                        {records.length > 0 ? records.map((student, i) => (
                            <tr key={student.batch_id} className={`border-b border-gray-100 hover:bg-purple-50 transition-all duration-300 ease-in-out ${i % 2 === 0 ? "bg-white" : "bg-[#efeded]"}`}>
                                <td className="py-3 px-6">
                                    <Link href={route('review.center.edit', student.batch_id)} className="inline-block px-4 py-1.5 rounded-[6px] bg-[#ffb736] text-white font-bold hover:bg-[#e0a800] hover:scale-105 hover:shadow-md transition-all duration-300 ease-in-out text-center">
                                        {student.student_number}
                                    </Link>
                                </td>
                                <td className="py-3 px-6 text-gray-800 uppercase font-bold">{student.name}</td>
                                <td className={`py-3 px-6 text-center font-bold ${student.review_center === '—' ? 'text-gray-400 font-normal' : 'text-[#5c297c]'}`}>
                                    {student.review_center}
                                </td>
                            </tr>
                        )) : (
                ...
                <ReviewCenterAddModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} currentFilter={filter} />
            </div>
        </AuthenticatedLayout>
    );
}