...
class LicensureExamController extends Controller
{
    ...
    public function update(Request $request, $batchId)
    {
        ...
        // 📝 AUDIT LOG
        AuditService::logStudentAcademic($student->student_number, "Updated Licensure Exam Result: {$validated['exam_result']} for Batch {$batch->batch_number} ({$batch->year})");
    ...
    public function import(Request $request)
    {
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            if ($batch) {
                ...
                // 📝 AUDIT LOG
                AuditService::logStudentAcademic($studentNumber, "Imported CSV Licensure Result: {$result}");
                $count++;
            }
        }
        fclose($handle);
